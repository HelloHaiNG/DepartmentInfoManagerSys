				...
				<TR>
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						<TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
							<TR>
								<TD class=manageHead>当前位置：公告中心 &gt; 公告列表</TD>
							</TR>
							<TR>
								<TD height=2></TD>
							</TR>
									...
									<TD>
										<TABLE id=grid
											...
											<TBODY>
												...
												<c:forEach items="${pageBean.listNotices }" var="notice">
													<TR
														style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none">
														<TD>${notice.ndate }</TD>
														<TD>${notice.ncontent }</TD>
														<TD><A
															href="${pageContext.request.contextPath }/notice_deleteByNid.action?nid=${notice.nid }">删除</A>
														</TD>
													</TR>
												</c:forEach>
											</TBODY>
									...
									<TD><SPAN id=pagelink>
											<DIV
												...
												<c:if test="${pageBean.currentPage!=1}">
												  [<A
														href="${pageContext.request.contextPath }/notice_all.action?currentPage=${pageBean.currentPage-1 }">前一页</A>]
												</c:if>
												<c:if test="${pageBean.currentPage!=pageBean.totalPage}">
												  [<A
														href="${pageContext.request.contextPath }notice_all.action?currentPage=${pageBean.currentPage+1 }">后一页</A>]
												</c:if>
												</c:if>
