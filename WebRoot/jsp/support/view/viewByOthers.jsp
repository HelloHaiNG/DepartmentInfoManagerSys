				...
				<TR>
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						<TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
							<TR>
								<TD class=manageHead>当前位置：支援中心 &gt; 支援列表</TD>
							</TR>
							<TR>
								<TD height=2></TD>
							</TR>
											...
											<TBODY>
												...
												<c:forEach items="${pageBean.listSupports }" var="support">
													<TR
														...
														<TD>${support.stime }</TD>
														<TD><A
															href="${pageContext.request.contextPath }/support_viewBySidByOthers.action?sid=${support.sid }">查看</A>
														</TD>
													</TR>
												</c:forEach>
											</TBODY>
									...
									<TD><SPAN id=pagelink>
											<DIV
												...
												<c:if test="${pageBean.currentPage!=1}">
												  [<A
															href="${pageContext.request.contextPath }/supprot_listByOthers.action?currentPage=${pageBean.currentPage-1 }">前一页</A>]
												</c:if>
													<c:if test="${pageBean.currentPage!=pageBean.totalPage}">
												  [<A
															href="${pageContext.request.contextPath }/supprot_listByOthers.action?currentPage=${pageBean.currentPage+1 }">后一页</A>]
												</c:if>
												</c:if>
